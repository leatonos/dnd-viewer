...
export default function DMCharacter({char, roomId, dmKey}: Props) {
  ...
  const [textColor,setTextColor] = useState<string>(char.text_color);
  const [bgColor,setBgColor] = useState<string>(char.text_color);
  ...
  return (
    <CharacterCard
      ...
      bgcolor={currentCharInfo.bg_color ?? "#222222"}
    >
        {/* Level Badge */}
        ...
        <SettingsButton onClick={() => setIsSettingsOpen(true)} title="Settings">
         ...
         {/* Character Image */}
        <CharacterPhoto bgcolor={currentCharInfo.bg_color ?? "#222222"} src={currentCharInfo.photo} alt={currentCharInfo.char_name} />
         {/* Name Editor */}
        <NameInput $nameColor={textColor} type="text" value={currentCharInfo.char_name} onChange={(e) => { updateStat("char_name", e.target.value) }}/>
        {/* HP Bar */}
        <FlexRow>
          <NumberButton $textColor={textColor} onClick={() => updateStat("hp", Math.max(currentCharInfo.hp - 1, 0))}>-</NumberButton>
          <ProgressBar value={currentHPPercent} barColor="#ef4444" textColor="#ffffff" label={HPLabel} isDM={true} />
          <NumberButton $textColor={textColor} onClick={() => updateStat("hp", Math.max(currentCharInfo.hp + 1, 0))}>+</NumberButton>
        </FlexRow>
        {/* Custom HP Input */}
        <FlexRow>
          <NumberButton $textColor={textColor} onClick={() => updateStat("hp", Math.max(currentCharInfo.hp - hpInput, 0))}>-</NumberButton>
          <InputField $textColor={textColor} type="text" value={hpInput} onChange={(e) => {setHpInput(e.target.value === "" ? 0 : parseInt(e.target.value, 10))}} />
          <NumberButton $textColor={textColor} onClick={() => updateStat("hp", Math.max(currentCharInfo.hp + hpInput, 0))}>+</NumberButton>
        </FlexRow>
        {/* MP Bar */}
        <FlexRow>
          <NumberButton $textColor={textColor} onClick={() => updateStat("mana", Math.max(currentCharInfo.mana - 1, 0))}>-</NumberButton>
          <ProgressBar value={currentMPPercent} barColor="#3b82f6" textColor="#ffffff" label={MPLabel} isDM={true} />
          <NumberButton $textColor={textColor} onClick={() => updateStat("mana", Math.max(currentCharInfo.mana + 1, 0))}>+</NumberButton>
        </FlexRow>
         {/* Custom MP Input */}
        <FlexRow>
          <NumberButton $textColor={textColor} onClick={() => updateStat("mana", Math.max(currentCharInfo.mana - mpInput, 0))}>-</NumberButton>
          <InputField $textColor={textColor} type="text" value={mpInput} onChange={(e) => {setMpInput(e.target.value === "" ? 0 : parseInt(e.target.value, 10))}} />
          <NumberButton $textColor={textColor} onClick={() => updateStat("mana", Math.max(currentCharInfo.mana + mpInput, 0))}>+</NumberButton>
        </FlexRow>
        {/* Experience Bar */}
        {currentCharInfo.required_exp > 0 && <>
        <FlexRow>
          <NumberButton $textColor={textColor} onClick={() => updateStat("exp", Math.max(currentCharInfo.exp - 1, 0))}>-</NumberButton>
          <ExperienceBar exp={currentCharInfo.exp} requiredExp={currentCharInfo.required_exp} isDM={true} />
          <NumberButton $textColor={textColor} onClick={() => updateStat("exp", Math.max(currentCharInfo.exp + 1, 0))}>+</NumberButton>
        </FlexRow>
        {/* Custom EXP Input */}
        <FlexRow>
          <NumberButton $textColor={textColor} onClick={() => updateStat("exp", Math.max(currentCharInfo.exp - expInput, 0))}>-</NumberButton>
          <InputField $textColor={textColor} type="text" value={expInput} onChange={(e) => {setExpInput(e.target.value === "" ? 0 : parseInt(e.target.value, 10))}} />
          <NumberButton $textColor={textColor} onClick={() => updateStat("exp", Math.max(currentCharInfo.exp + expInput, 0))}>+</NumberButton>
        </FlexRow>
        </>}
...
const NumberButton = styled.button<{ $textColor?: string }>`
  width: 28px;
  height: 28px;
  margin: 0px 5px;
  cursor: pointer;
...
const InputField = styled.input<{ $textColor?: string }>`
  width: 60px;
  text-align: center;
  color: ${({ $textColor }) => $textColor || "#000000"};
 ...
 const NameInput = styled.input<{ $nameColor?: string }>`
  ...
  width: auto;
  cursor: text;
  color: ${({ $nameColor }) => $nameColor || "#000000"};
